...
def align_images(images, manual=False, template_top_left_x=0,
    ...
    if manual:
        ...
        # find a bright patch in the reference color layer (blue)
        width = 50
...
def shift_color_chanel(offset, images, color_chanel):
    ...
    for image in images:
        ...
        if offset[1] < 0:
            # go down
            layer = np.pad(layer, ((abs(offset[1]), 0), (0, 0)), mode='constant')[:, :offset[1]]
        elif offset[1] > 0:
            # go up
            layer = np.pad(layer, ((0, offset[1]), (0, 0)), mode='constant')[offset[1]:, :]

        # horizantal
        if offset[0] < 0:
            # shift right
            layer = np.pad(layer, ((0, 0), (abs(offset[0]), 0)), mode='constant')[:, :offset[0]]
        elif offset[0] > 0:
            # shift left
            layer = np.pad(layer, ((0, 0), (0, offset[0])), mode='constant')[:, offset[0]:]
