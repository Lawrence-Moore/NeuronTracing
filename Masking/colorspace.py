...
class colorSpaces():
    def __init__(self, colorspace, intensitylabel, intensityslider,
                 volumeselect, modemenu, areaselectionview):
        ...
        # (changing polygon's shape) vs. 'auto' vs. 'circular'
        ...
        self.modeMenu = modemenu  # QComboBox: dropdown to select drawing mode
    ...
    def modeChange(self, text):
        '''
        :param text: the current string self.modeMenu
        :return: none: upon change in self.modeMenu, translate label text into
    ...
    def circularCreate(self):
        ...
        for n in xrange(0, numNodes):  # find and save location of nodes
            ...
            if not (3 < x < self.side - 3 and 3 < y < self.side - 3):
                ...
                'boundign nodes inside the colorspace window. Please debug...'))
                error.exec_()
                return
            self.currentArea.append([x, y])
    ...
    def createValidityMap(self, push=True):  #vxy
        ...
        for i, area in enumerate(self.areas): # preprocessing areas into dicts
            if area:
                ...
                minx = self.side
                maxx = 0
                for (x, y) in sorted(zip(area[0], area[1])): # sort by the x's
